...
export default function Sequences({ slug }:{ slug:string }) {
  ...
  const [q, setQ] = useState(""); const [working, setWorking] = useState<string[]>([]);

  useEffect(()=>writeJSON(LS.sequences, sequences), [sequences, LS.sequences]);
  useEffect(()=>{ const onS = (e:StorageEvent)=>{ if(e.key===LS.catalog) setCatalog(readJSON(LS.catalog, [])); }; window.addEventListener("storage",onS); return ()=>window.removeEventListener("storage",onS); },[LS.catalog]);

  const byBKey = useMemo(()=>catalogMap(catalog), [catalog]);
  const palette = useMemo(()=>{
    const s=q.trim().toLowerCase(); if(!s) return catalog;
  ...
  const commit = (next:string[][])=>{ setSequences(next); writeJSON(LS.sequences, next); };

  const addStep =(bKey:string)=> setWorking(w=> w.includes(bKey) ? w : [...w,bKey]);
  const move = (i:number, d:number)=> setWorking(w=>{ const j=i+d; if(j<0||j>=w.length) return w; const a=w.slice(); [a[i],a[j]]=[a[j],a[i]]; return a; });
  const remove = (i:number)=> setWorking(w=>w.filter((_x,idx)=>idx!==i));
  const clear = ()=> setWorking([]);

  const save = ()=>{
    if(!working.length) return;
    if(hasDup){ alert("Sequence has duplicate steps."); return; }
    if(sequences.some(p=>pathsEqual(p,working))){ alert("This exact sequence already exists."); return; }
    commit([working.slice(), ...sequences]); clear();
  };
  const del=(idx:number)=> commit(sequences.filter((_x,i)=>i!==idx));

  const exportCSV = ()=>{
    const rows = ["path"]; sequences.forEach(p=>rows.push(JSON.stringify(p.join("-"))));
    const csv = rows.join("\n");
    const a = document.createElement("a");
    a.href = "data:text/csv;charset=utf-8,"+encodeURIComponent(csv);
    a.download = `${slug}-sequences.csv`; a.click();
  };
  const importCSV = ()=>{
    const txt = prompt("Paste CSV with header: path"); if(!txt) return;
    ...
    const arr = (first==="path" ? lines.slice(1) : lines)
      ...
      .filter(p=>p.length);
    const next = sequences.slice(); arr.forEach(p=>{ if(!next.some(q=>pathsEqual(q,p))) next.push(p); });
    commit(next);
  };
  ...
  return (
    <div style={{ display:"grid", gap:12 }}>
      ...
      <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:12, padding:12 }}>
        <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:8 }}>
          <b>Build sequence</b>
          <Button onClick={save} disabled={!working.length || hasDup}>Save</Button>

...
export default function BCEditor({ slug }:{ slug:string }) {
  ...
  const usedSet = useMemo(()=>{
    ...
    (catalog||[]).forEach(r => { if (r?.bVal) s.add(norm(r.bVal)); if (r?.cVal) s.add(norm(r.cVal)); });
    return s;
  ...
  function updateVal(bKey:string, which:"bVal"|"cVal", val:string) {
    setCatalog(prev=>{
      ...
      if (enforceNoMatch && nextRow.bVal && nextRow.cVal && norm(nextRow.bVal)===norm(nextRow.cVal)) {
        alert("value-1 and value-2 cannot be the same."); return prev;
      }
      if (val && currentUsed.has(norm(val))) { alert("This value already exists across B/C."); return prev; }

      if (idx>=0) a[idx]=nextRow; else a.push(nextRow);
      return a;
    });
  }
  ...
  function quickAddWordsForRow(bKey:string) {
    ...
    if (s===null) return;
    const words = parseWords(s); if (!words.length) { alert("No words entered."); return; }

    const sequences:string[][] = readJSON(LS.sequences, []);
    const starting = (sequences||[]).filter(p => Array.isArray(p) && p[0]===bKey);
    if (!starting.length) { alert("No sequences start with this row. Save sequences first."); return; }

    // mark a flag in entry LS so Entry page can pick them up (we’ll create blocks there)
    ...
    starting.forEach(path=>{
      ...
      words.forEach(w=>{
        ...
        blocks.push({ block: nb, rows: path.map((bk,i)=>({ tokenIndex:i, token:(m.get(bk)?.bVal||bk), block:nb, dec:1, output: i===0 ? w : "enter output value" }))});
      });
  ...
  const pageRows = useMemo(()=>{ const out:number[]=[]; for(let i=start;i<start+perPage;i++) out.push(i); return out; },[start,perPage]);
    ...
    <div style={{ display:"grid", gap:12 }}>
      <div style={{ display:"flex", gap:10, alignItems:"center" }}>
        ...
        <label style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
          <input type="checkbox" checked={enforceNoMatch} onChange={(e)=>setEnforceNoMatch(e.target.checked)} /> Enforce B≠C
        </label>
        <label style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center" }}>
        <div>Start index</div>
        <Input value={start} onChange={e=>setStart(Math.max(1, Number(e.target.value)||1))} style={{ width:80 }} />
        <div>Rows per page</div>
        <select value={perPage} onChange={e=>setPerPage(Number(e.target.value))} style={{ padding:6, border:"1px solid #d1d5db", borderRadius:8 }}>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead>
            <tr style={{ background:"#f8fafc" }}>
              <th style={{ textAlign:"left", padding:8, borderBottom:"1px solid #e5e7eb", width:60 }}>#</th>
              <th style={{ textAlign:"left", padding:8, borderBottom:"1px solid #e5e7eb", width:180 }}>column B- label</th>
              <th style={{ textAlign:"left", padding:8, borderBottom:"1px solid #e5e7eb" }}>value-1</th>
              <th style={{ textAlign:"left", padding:8, borderBottom:"1px solid #e5e7eb", width:200 }}>column C- label</th>
              <th style={{ textAlign:"left", padding:8, borderBottom:"1px solid #e5e7eb" }}>value-2</th>
            </tr>
          </thead>
          <tbody>
            {pageRows.map(i=>{
              const bKey = `column B-${i}`; const cKey = `column C-${i}`;
                ...
                <tr key={i}>
                  ...
                  <td style={{ padding:8, borderBottom:"1px solid #f1f5f9" }}>
                    <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                      <Input value={rec.bVal} onChange={e=>updateVal(bKey,"bVal",e.target.value)} placeholder="add value-1" />
                      {i===1 && <Button title="Quick add headword(s) for sequences starting here" onClick={()=>quickAddWordsForRow(bKey)}>+ words</Button>}
                    </div>
                  </td>
                  <td style={{ padding:8, borderBottom:"1px solid #f1f5f9", color:"#111827" }}>{cKey}</td>
                  <td style={{ padding:8, borderBottom:"1px solid #f1f5f9" }}>
                    <Input value={rec.cVal} onChange={e=>updateVal(bKey,"cVal",e.target.value)} placeholder="add value-2" />
                  </td>
                </tr>
              );
            })}
